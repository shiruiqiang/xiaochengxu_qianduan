...
Page({
  ...
  bindGetUserInfo (e) {
    console.log("0000000000000000000000000   " + JSON.stringify(e.detail.userInfo))
    this.setData({
      step: 2
    })
  },
  getPhoneNumber (e) {
    ...
    wx.request({
      url: t.server_url + '/mini_program/getPhoneNumber', //仅为示例，并非真实的接口地址
      method: 'GET',
      ...
      success (res) {
        ...
        if(res.data.code == 0){
          wx.reLaunch({
            url: '/pages/register/step/register'
          })
        }else{
          wx.showToast({
            title: '授权失败',
